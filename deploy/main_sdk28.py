...
class MainWindow(QMainWindow):
    def __init__(
        ...
        self.llm_log_file = None  # LLM请求响应日志文件路径
        ...
        self.video_writer = None  # 流式写入的VideoWriter
        self.video_frame_count = 0  # 记录写入的帧数
    ...
    def _perform_rest(self):
        ...
        try:
            ...
            # 1. 点击屏幕中心（暂停视频）
            self.log_info(f"👆 点击屏幕中心 ({center_x}, {center_y}) 暂停")
            self.u2_device.click(center_x, center_y)
            time.sleep(0.5)
            ...
            # 3. 点击屏幕中心（恢复播放）
            self.log_info(f"👆 点击屏幕中心 ({center_x}, {center_y}) 恢复")
            self.u2_device.click(center_x, center_y)
            time.sleep(0.5)
    ...
    def test_model_connection(self):
        ...
        try:
            ...
            user_prompt = get_user_prompt(
                ...
                persona=test_persona if use_persona else None
            )
    ...
    def start_interaction(self):
        ...
        # 使用persona作为父目录，每个session有独立的子目录
        persona_dir = deploy_log_dir / persona_name
        persona_dir.mkdir(exist_ok=True, parents=True)
        
        # 每个session独立的目录
        self.session_dir = persona_dir / f"session_{session_id}"
        ...
        (self.session_dir / "llm_logs").mkdir(exist_ok=True)
        
        # 初始化LLM日志文件
        self.llm_log_file = self.session_dir / "llm_logs" / f"llm_log_{session_id}.jsonl"
    ...
    def stop_interaction(self):
        ...
        if self.session_data:
            ...
            # 保存最终会话文件（保存为metadata.json）
            session_file = self.session_dir / "metadata.json"
    ...
    def _log_llm_interaction(self, request_data, response_data, metadata):
        """记录LLM的请求和响应到单独的日志文件
        
        Args:
            request_data: 请求数据，包含system_prompt, user_prompt等
            response_data: 响应数据
            metadata: 元数据，包含timestamp, step, model_name, llm_delay等
        """
        ...
        log_entry = {
            ...
            "llm_delay": round(metadata.get("llm_delay", 0), 3),
            "request": {
                "system_prompt": request_data.get("system_prompt"),
                "user_prompt": request_data.get("user_prompt"),
                "history_screenshots": request_data.get("history_screenshots", []),
                "current_screenshot": request_data.get("current_screenshot"),
                "image_quality": request_data.get("image_quality"),
                "max_history": request_data.get("max_history"),
                "use_persona": request_data.get("use_persona"),
            },
            "response": {
    ...
    def _get_ai_response(self, screenshot_path):
        ...
        user_prompt = get_user_prompt(
            ...
            audio_transcript=None,
            persona=self.ai_persona if use_persona else None
        )
        ...
        request_data = {
            ...
            "history_screenshots": self.ai_history_screenshots[-current_max_history:] if self.ai_history_screenshots else [],
            "current_screenshot": screenshot_path,
            "image_quality": image_quality,
            "max_history": current_max_history,
        ...
        self._log_llm_interaction(request_data, response_data, metadata)
            ...
            if self.is_recording:
                ...
                if self.video_writer:
                    try:
                        ...
                        # pts时间戳错误时只记录日志，不停止录制
                        # 常见错误: "Invalid pts (N) <= last (N)" 
                        # 这是因为scrcpy流中偶尔有重复时间戳的帧
                        pass  # 静默忽略单帧写入错误，继续录制
            
    def update_window_size(self, display_width, display_height):
        # 新布局：第一列(320) + 第二列(260) + 屏幕 + 日志(400) + 间距
        column1_width = 320
        column2_width = 260
        log_panel_width = 400
        margin = 50
        window_width = column1_width + column2_width + display_width + log_panel_width + margin
